...
class TestRecursosEducativosCarga(unittest.TestCase):
    """
    Pruebas de carga para el servicio de presentar portadas de recursos educativos
    """
    
    BASE_URL = "http://localhost:5001/recurso_educativo_routes"  
    ENDPOINT_PORTADAS = f"{BASE_URL}/portadas_recursos_educativos"
    ...
    def generar_payload_aleatorio(numero_usuario):
        """Genera un payload aleatorio para consultar portadas de recursos educativos"""
        id_usuario = TestRecursosEducativosCarga.USUARIOS_PRUEBA[numero_usuario % len(TestRecursosEducativosCarga.USUARIOS_PRUEBA)]
    ...
    @staticmethod
    def consultar_portadas_individual(url, payload, numero_intento):
        """Consulta portadas de recursos educativos de forma individual y retorna el resultado"""
        try:
            inicio = time.time()
    ...
    def simular_consultas_simultaneas(self, cantidad_usuarios):
        """
        Simula consultas simultáneas al servicio de portadas de recursos educativos
        ...
        print(f"Endpoint: {self.ENDPOINT_PORTADAS}")
        print(f"{'='*80}\n")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=cantidad_usuarios) as executor:
            futures = [
                executor.submit(
                    self.consultar_portadas_individual,
                    self.ENDPOINT_PORTADAS,
                    payloads[i],
                    i + 1
                )
...
if __name__ == "__main__":
    ...
    # test = TestRecursosEducativosCarga()
