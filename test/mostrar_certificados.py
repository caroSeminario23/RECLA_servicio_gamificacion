...
class TestCertificadosCarga(unittest.TestCase):
    """
    Pruebas de carga para el servicio de obtener certificados con estado
    """
    
    BASE_URL = "http://localhost:5001/certificado_routes"  
    ENDPOINT_CERTIFICADOS = f"{BASE_URL}/get_certificados_con_estado"
    ...
    def generar_payload_aleatorio(numero_usuario):
        """Genera un payload aleatorio para consultar certificados"""
        id_usuario = TestCertificadosCarga.USUARIOS_PRUEBA[numero_usuario % len(TestCertificadosCarga.USUARIOS_PRUEBA)]
    ...
    @staticmethod
    def consultar_certificados_individual(url, payload, numero_intento):
        """Consulta certificados de forma individual y retorna el resultado"""
        try:
            inicio = time.time()
    ...
    def simular_consultas_simultaneas(self, cantidad_usuarios):
        """
        Simula consultas simultáneas al servicio de certificados
        ...
        print(f"Endpoint: {self.ENDPOINT_CERTIFICADOS}")
        print(f"{'='*80}\n")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=cantidad_usuarios) as executor:
            futures = [
                executor.submit(
                    self.consultar_certificados_individual,
                    self.ENDPOINT_CERTIFICADOS,
                    payloads[i],
                    i + 1
                )
